...
class PredictorGUI:
    def __init__(self):
        ...
        self.log = Text(frame_logs, height=10, width=180)  
        ...
        self.log1 = Text(frame_logs, height=40, width=60)
        self.log1.pack(side="left", fill="both", expand=True)

        self.log2 = Text(frame_logs, height=40, width=60) 
        self.log2.pack(side="left", fill="both", expand=True)

        self.log3 = Text(frame_logs, height=40, width=60)  
    ...
    def load_folder(self):
        ...
        if folder_path:
            ...
            for path in self.csv_files:
                ...
                display_name = f"{heure} / {nb_scores} | {teamA_name} VS {teamC_name}"
    ...
    def load_csv(self, path):
        ...
        self.log.insert(END, f"Scores historiques : {nb_scores}\n")
        self.log.insert(END, f"Teams → A: {teamA_name} | C: {teamC_name}\n")

        self.teamA_scores = [str(v).strip() for v in self.data.iloc[:, 0].tolist()]
        self.teamC_scores = [str(v).strip() for v in self.data.iloc[:, 2].tolist()]
    ...
    def predict(self):
        ...
        try:
            ...
            self.log.insert('end', f"\nRésultat → ⚽= {col_e1} - {col_g1}\n")
        except IndexError:
            self.log.insert('end', "Résultat → ⚽= non renseigné\n")

        self.log.insert('end', f"\nDerniers scores individuels→ ⚽= A: {self.team1_series[-1]} || C: {self.team2_series[-1]}\n")
