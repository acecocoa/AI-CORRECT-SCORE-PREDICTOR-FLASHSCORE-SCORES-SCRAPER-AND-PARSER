...
#===
    ...
    def log_motif_result(self, log_widget, title, motif_type, result):
        log_widget.insert(tk.END, f"{title}\n")
        #log_widget.insert(tk.END, f"Motif type : {motif_type}\n")

        if result["motif_cible"] is None:
            log_widget.insert(tk.END, "⚽=⛔😡\n")
            return

        #log_widget.insert(tk.END, f"Motif cible : {result['motif_cible']}\n")
        log_widget.insert(
            tk.END,
            f"⚽={result['prediction']} | 🟰={len(result['similar_blocks'])}\n"
        )
    ...
    def run_prediction(self,motif_length=2):
        ...
        def write_method3(series,log):
            ...
            self.write_log(log,"\n"+row("Id",peak_Id))
            self.write_log(log, "\n" + row("Pr", peak_Pr) + "\n\n")
            self.write_log(log, "=========== CORRECT SCORE SET 1 ===========\n")
            ...
            for title, ml, bloc in motif_configs:
                ...
                self.log_motif_result(log, "📊 Motif croissant:", "croissant", res_croissant)
                ...
                self.log_motif_result(log, "📊️ Motif décroissant:", "decroissant", res_decroissant)

                res_prop = self.detect_target_motif_prediction_disorder(
                    series[:-1], motif_length=ml, block_size=bloc, motif_type="proportionnel"
                )
                self.log_motif_result(log, "📊 Motif proportionnel:", "proportionnel", res_prop)
