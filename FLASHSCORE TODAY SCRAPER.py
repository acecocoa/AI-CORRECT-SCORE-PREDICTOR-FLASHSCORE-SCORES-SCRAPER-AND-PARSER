...
class FlashscoreGUI(tk.Tk):
    ...
    def run_scores(self):
        ...
        # si annulation (bouton cancel)
        if selection is None:
            self.log_msg("[INFO] Opération annulée")
            return

        # vide → tous les matchs
        if selection.strip() == "":
            selected_matches = self.scanned_matches
            self.log_msg("[INFO] Aucun filtre → tous les matchs seront traités")

        else:
            try:
                indexes = [int(x.strip()) for x in selection.split(";") if x.strip().isdigit()]
            except:
                messagebox.showerror("Erreur", "Format invalide. Exemple: 1;5;8")
                return

            selected_matches = []
            for i in indexes:
                if 1 <= i <= len(self.scanned_matches):
                    selected_matches.append(self.scanned_matches[i-1])
                else:
                    self.log_msg(f"[WARN] Match {i} inexistant → ignoré")

            if not selected_matches:
                self.log_msg("[ERROR] Aucun match valide sélectionné")
                return
        ...
        async def runner_all():
            async with async_playwright() as p:
                ...
                for idx, m in enumerate(selected_matches, start=1):
                    ...
                    self.log_msg(f"[MATCH {idx}/{len(selected_matches)}] {time_match}")
        ...
        self.log_msg("[FIN] Traitement terminé pour les matchs sélectionnés")
