...
class FlashscoreGUI(tk.Tk):
    def __init__(self):
        ...
        tk.Label(frame_time_day, text="<= ALL EMPTY= ALL & TODAY").grid(row=0, column=6, sticky="w")
        ...
        tk.Button(frame_buttons, text="GET SCORES", command=self.run_scores).grid(row=0, column=2, padx=5)
        ...
        tk.Label(self, text="SELECTED DAY LEAGUES LIST").pack(anchor="w", padx=10)
    ...
    def run_scores(self):
        ...
        try:
            ...
            self.log_msg(f"[OK] {len(self.scanned_matches)} x2 scores match(s) saved.")
        except Exception as e:
    ...
    def show_matches(self):
        ...
        self.log_msg(f"[INFO] {len(self.scanned_matches)} matchs trouvés")

        for i,m in enumerate(self.scanned_matches,1):

            self.log_msg(f"[MATCH {i}] {m['time']} → URL duel: {m['url_duel']}")
    ...
    def scan_leagues(self):
        ...
        try:
            ...
            self.log_msg(f"[OK] {len(leagues_dict)} ligues détectées")
